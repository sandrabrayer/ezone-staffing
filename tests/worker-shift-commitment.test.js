...
test('HEADERS_WORKERS keeps shift_commitment at index 4 (append-only invariant)', () => {
  ...
  // value ever shifts.
  assert.deepStrictEqual(cols, ['id', 'name', 'notes', 'created_at', 'shift_commitment', 'start_date', 'gmach_month', 'phone', 'payroll_emp_number'],
  ...
  assert.strictEqual(cols[5], 'start_date',
  ...
  assert.strictEqual(cols[8], 'payroll_emp_number',
    'payroll_emp_number must stay at index 8 (readWorkersSafe reads r[8] for it)');
  assert.strictEqual(cols[cols.length - 1], 'payroll_emp_number',
    'payroll_emp_number is the newest column and must be LAST');
});
