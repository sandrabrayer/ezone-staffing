...
function seedReaders(ctx) {
  ctx.readWorkersSafe = () => [
    ...
    { id: 'w8', name: 'תמר גל', notes: '', startDate: '2026-05-01' },
  ];
...
test('feed filters to supervision roles, maps active status, keeps empty start dates, skips orphans', () => {
  ...
  seedReaders(ctx);
  const guides = plain(ctx.computeGuidesForHadrachot_());
  // w7 (טבח/ית) and w8 (plain מטפל/ת) are not supervision roles and a5 is
  // orphaned — all out.
...
test('HEADERS_WORKERS is untouched — the feed is read-only, no schema change', () => {
  ...
  assert.deepStrictEqual(cols,
    ['id', 'name', 'notes', 'created_at', 'shift_commitment', 'start_date', 'gmach_month', 'phone', 'payroll_emp_number'],
    'HEADERS_WORKERS is append-only and position-mapped — the feed itself must not touch it (gmach_month was appended by the final_settlement feature, phone by the coordinators feed, payroll_emp_number by בקרת שכר)');
});
