...
(function (root, factory) {
  ...
  // Which of the engine's three money buckets a line landed in. `source`
  // says how the number was priced; `bucket` says how far it can be
  // trusted, and a report that is meant to be reconciled needs the latter.
  // How a start date was arrived at, in words. A blank source means a
  // person entered the date; 'payroll_floor' means it was reconstructed
  // from the first month the worker appears in the payroll book, so the
  // true start is that month or earlier. An exported date must never look
  // more certain on a spreadsheet than it does on the screen.
  const START_DATE_SOURCE_LABELS = {
    payroll_floor: 'משוער — לפי החודש הראשון בדוח שכר',
  };
  function startDateSourceLabel(w) {
    if (!w || !String(w.startDate || '').trim()) return '';
    const src = String(w.startDateSource || '').trim();
    if (!src) return 'הוזן ידנית';
    return START_DATE_SOURCE_LABELS[src] || src;
  }

  const BUCKET_LABELS = {
    confirmed: 'מאושר בפועל',
    estimated: 'אומדן',
    missing: 'חסר תאריך תחילה',
    none: 'חסרים נתונים',
  };
  function inBucket(line, bucket) {
    return line.bucket === bucket ? line.cost : 0;
  }
  ...
  const REPORTS = {
    ...
    payroll: {
      ...
      build(ctx) {
        ...
        const headers = ['חודש', 'בית', 'שם', 'תפקיד', 'סוג העסקה', 'סטטוס',
          'עלות', 'מקור הנתון', 'שיוך הסכום', 'ימים שחויבו', 'ימים בחודש',
          'חוקיות החישוב', 'פירוט החישוב', 'נתונים חסרים', 'workerId', 'assignmentId'];
        const rows = assignmentLines(report, workersById).map(l => [
          ...
          BUCKET_LABELS[l.bucket] || l.bucket,
          l.daysCounted,
          l.daysInMonth,
          l.rule,
          l.basis,
    ...
    actualVsEstimate: {
      ...
      build(ctx) {
        ...
        const headers = ['חודש', 'בית', 'שם', 'סוג העסקה', 'עלות',
          'מאושר בפועל', 'אומדן', 'חסר תאריך תחילה', 'חסרים נתונים',
          'חוקיות החישוב', 'workerId', 'assignmentId'];
        const rows = assignmentLines(report, workersById).map(l => [
          ctx.month,
          houseLabel(ctx.houseNames, l.house),
          nameOf(workersById, l.workerId),
          EMPLOYMENT_TYPE_LABELS[l.employmentType] || l.employmentType,
          l.cost,
          inBucket(l, 'confirmed'),
          inBucket(l, 'estimated'),
          inBucket(l, 'missing'),
        ...
        // A totals row, because this report exists to be reconciled. The
        // three money columns sum to the cost column, line by line and in
        // the total — that is the whole point of the third bucket.
        rows.push(['סה״כ', '', '', '',
          report.totals.projectedTotal,
          report.totals.actualConfirmed,
          report.totals.estimated,
          report.totals.missingDataCost,
    ...
    byHouse: {
      ...
      build(ctx) {
        ...
        const headers = ['חודש', 'בית', 'שיבוצים', 'עלות', 'מאושר בפועל', 'אומדן',
          'חסר תאריך תחילה', 'שיבוצים ללא נתונים מלאים', 'מתוכה עלות מדריכים',
          'תקציב', 'תקציב מדריכים', 'יתרה', 'ניצול באחוזים'];
        ...
        rows.push([ctx.month, 'סה״כ רשת',
          ...
          report.totals.estimated, report.totals.missingDataCost,
          report.totals.missingData, '', '', '', '', '']);
        return { headers, rows };
      },
    },
    ...
    byRole: {
      ...
      build(ctx) {
        ...
        report.lines.filter(l => l.kind === 'assignment').forEach(l => {
          ...
          if (!acc[k]) acc[k] = { count: 0, cost: 0, actual: 0, estimate: 0, noStart: 0, missing: 0 };
          acc[k].count++;
          acc[k].cost += l.cost;
          acc[k].actual += inBucket(l, 'confirmed');
          acc[k].estimate += inBucket(l, 'estimated');
          acc[k].noStart += inBucket(l, 'missing');
          if ((l.missingData || []).length) acc[k].missing++;
        });
        const headers = ['חודש', 'תפקיד', 'שיבוצים', 'עלות', 'מאושר בפועל', 'אומדן',
          'חסר תאריך תחילה', 'שיבוצים ללא נתונים מלאים'];
        const rows = Object.keys(acc).sort().map(k => [
          ctx.month, k, acc[k].count, acc[k].cost, acc[k].actual, acc[k].estimate,
          acc[k].noStart, acc[k].missing,
        ]);
        return { headers, rows };
      },
    },
    ...
    unassigned: {
      ...
      build(ctx) {
        ...
        const headers = ['שם', 'טלפון', 'תאריך תחילת עבודה', 'מקור התאריך', 'מצב', 'workerId'];
        const rows = (ctx.workers || [])
          .filter(w => w && !placed[w.id])
          .sort((a, b) => String(a.name).localeCompare(String(b.name)))
          .map(w => [
            w.name, w.phone || '', w.startDate || '', startDateSourceLabel(w),
    ...
    roster: {
      ...
      build(ctx) {
        ...
        const headers = ['בית', 'שם', 'טלפון', 'תפקיד', 'פירוט תפקיד', 'סוג העסקה',
          'קטגוריה', 'סטטוס', 'תאריך תחילת עבודה', 'מקור התאריך',
          'עלות בחודש הנבחר', 'workerId', 'assignmentId'];
        const rows = assignmentLines(report, workersById).map(l => {
