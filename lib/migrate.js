...
// per_session 3-rate migration: copy the legacy single rate/sessions pair
// into the new `individual` pair, leaving group + external at 0. The
// individual bucket is the natural home for a therapist's existing sessions.
// This is the pure mapping tested here and mirrored by
// migratePerSessionRatesToThreeRate() in apps-script/Code.gs. Non-negative,
// integer-rounded — a garbage/negative legacy value maps to 0 rather than
// poisoning the new columns.
function perSessionRatesToThreeRate(legacyRate, legacySessions) {
  const rate = Math.max(0, Math.round(Number(legacyRate) || 0));
  const sessions = Math.max(0, Math.round(Number(legacySessions) || 0));
  return {
    rateIndividual: rate,
    sessionsIndividual: sessions,
    rateGroup: 0,
    sessionsGroup: 0,
    rateExternal: 0,
    externalPatients: 0,
