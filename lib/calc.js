...
// A new guide must complete their first hadracha within this many days of
// their employment start date; strictly MORE than this → alert.
...
//   overdue      — active guides (role מדריך/ה, assignment status active)
//                  whose start_date is MORE than FIRST_HADRACHA_GRACE_DAYS
//                  days ago and whose name is not in completedNames.
//                  Exactly 7 days is NOT overdue (strict "exceeds").
//   missingStart — active guides with an empty start_date (a separate
//                  data-hygiene warning; they can never be judged overdue).
// A guide placed at two houses is counted once. completedNames must be the
...
function firstHadrachaFlags(workers, assignments, completedNames, today) {
  ...
  const missingStart = [];
  (assignments || []).forEach(function (a) {
    if (!isInstructorRole(a.role)) return;
...
const API = {
  ...
  FIRST_HADRACHA_GRACE_DAYS, daysSince, normalizeGuideName,
