...
// per_case_commission: a marketer's commission per case — freelance, and with
// no fixed monthly amount (base cost 0; only an allowance can add to it).
const FREELANCER_TYPES = ['per_session', 'fixed_retainer', 'per_case_commission'];
...
// maternity) and 'chlt' (חל"ת, extended sick leave) mean the worker is
// NOT paid while on leave — their assignment cost counts as 0.
// 'final_settlement' (גמ"ח) means the worker finished with a final
// settlement: unpaid from the moment it's set (cost 0), still shown on the
// roster until the end of the month it was set (worker.gmachMonth), then
// listed with the finished workers. The stored salary is never overwritten
// — it's zeroed at computation time only, so reverting the status restores
// the original cost.
const WORKER_STATUSES = ['active', 'chld', 'chlt', 'final_settlement'];
const LEAVE_STATUSES = ['chld', 'chlt'];
const FINAL_SETTLEMENT_STATUS = 'final_settlement';
// Statuses whose assignment cost is 0 (leave + final settlement).
...
// True when the assignment's worker is on unpaid leave (חל"ד / חל"ת).
function isOnLeave(a) {
  return LEAVE_STATUSES.indexOf(workerStatus(a)) >= 0;
}

// True when the assignment's worker finished with a final settlement (גמ"ח).
function isFinalSettlement(a) {
  return workerStatus(a) === FINAL_SETTLEMENT_STATUS;
}

// True when the assignment's worker is not paid (leave or final settlement).
function isUnpaid(a) {
  return UNPAID_STATUSES.indexOf(workerStatus(a)) >= 0;
}
...
// A worker on חל"ד / חל"ת, or finished with a final settlement (גמ"ח), is
// not paid, so their cost is 0 (salary AND allowance both stop). The stored
// terms are untouched — only the computed cost is zeroed.
function assignmentCost(a) {
  if (!a) return 0;
  if (isUnpaid(a)) return 0;
  return assignmentBaseCost(a) + allowanceValue(a);
}

// ---------- monthly actuals (hourly / per_session) ----------
//
// DEPRECATED for the cost views. Every cost number the UI shows now comes
// from lib/cost-engine.js — ONE costForMonth() call per render — because
// the helpers below are month-blind in ways that produced the bug where
// changing the cost month moved the label and nothing else:
// monthlyAssignmentCost takes no month at all, no cost helper here reads
// startDate, and pendingHouseCost / coverageExtra are evaluated against
// `today` rather than the selected month.
//
// They are kept, unchanged and still tested, because other callers and the
// existing test suite depend on them. DO NOT add new cost logic here — add
// it to the engine, where there is one rule per decision and every line
// carries a trace. See docs/COST_RULES.md.
...
// Leave (חל"ד / חל"ת) still zeroes the cost, ahead of any actual/estimate.
...
function monthlyAssignmentCost(a, actual) {
  ...
  if (isUnpaid(a)) return { cost: 0, isEstimate: false };
...
// ---------- final settlement (גמ"ח) roster logic ----------
// worker.gmachMonth ('YYYY-MM') records the month the final_settlement
// status was set (written automatically by the backend). Roster rule:
//   - while the current month EQUALS gmachMonth → the worker still shows on
//     the roster (מצבת העובדים) with a visible גמ"ח badge;
//   - from the FOLLOWING month onward → dropped from the active roster and
//     listed with the finished workers instead (same place terminated
//     workers show). Plain 'YYYY-MM' string comparison — no date parsing,
//     no manual second step.

// True when the worker's final-settlement month has passed (month is the
// current 'YYYY-MM'). A blank/missing gmachMonth is never "finished" — the
// backend sets it when the status is applied, but a row saved by an older
// client stays visible rather than silently vanishing.
function gmachFinished(worker, month) {
  const gm = String((worker && worker.gmachMonth) || '').trim();
  if (!gm) return false;
  return String(month || '') > gm;
}

// Assignments that should still appear on the active roster: everything
// except final_settlement assignments whose worker's gmachMonth has passed.
function filterGmachFinished(assignments, workers, month) {
  const workerById = {};
  (workers || []).forEach(function (w) { if (w) workerById[w.id] = w; });
  return (assignments || []).filter(function (a) {
    if (!a) return false;
    if (!isFinalSettlement(a)) return true;
    return !gmachFinished(workerById[a.workerId], month);
  });
}

// The complement: final_settlement assignments whose month has passed —
// the rows the finished-workers view (ארכיב עובדים) lists alongside the
// terminated workers. Each row carries the worker's name + gmachMonth so
// the view can render without another join.
function gmachFinishedRows(assignments, workers, month) {
  const workerById = {};
  (workers || []).forEach(function (w) { if (w) workerById[w.id] = w; });
  return (assignments || []).filter(function (a) {
    return a && isFinalSettlement(a) && gmachFinished(workerById[a.workerId], month);
  }).map(function (a) {
    ...
    return {
      ...
      gmachMonth: String((w && w.gmachMonth) || '').trim(),
    };
  });
}

// ---------- instructor (מדריך/ה) cost ----------
// The exact role string stored for an instructor — mirror of the מדריך/ה
// option in ROLE_OPTIONS (lib/validate.js and index.html). An assignment
// counts toward the instructors budget line iff its role matches this.
...
// Instructor (מדריך/ה) assignments at a house.
function instructorAssignments(assignments, house) {
  return assignmentsByHouse(assignments, house).filter(a => isInstructorRole(a.role));
}

// Month-aware instructor cost for a house: the sum of monthlyAssignmentCost
// over the house's instructor (מדריך/ה) assignments — actuals where present,
// the estimate otherwise. Returns { cost, isEstimate }, where isEstimate is
// true when ANY contributing instructor fell back to its estimate (so the UI
// can flag the total with an "אומדן" badge). Coverage extras and pending
// terminations are deliberately NOT included — this is strictly the cost of
// instructor assignments AT the house, matching the "sum of costs for workers
// whose role is מדריך in that house" contract.
function houseMonthlyInstructorsCost(assignments, actualsIndex, house, month) {
  let cost = 0;
  let isEstimate = false;
  instructorAssignments(assignments, house).forEach(a => {
    const mc = monthlyAssignmentCost(a, lookupActual(actualsIndex, a.id, month));
    cost += mc.cost;
    if (mc.isEstimate) isEstimate = true;
  });
  return { cost: cost, isEstimate: isEstimate };
}
...
// True when a budget row carries a usable (non-blank) instructors budget.
// The instructors line is OPTIONAL: a row written before the total/instructors
// split — or a house whose budget has no instructors sub-line — leaves it
// blank/null. Backward compatibility: an existing single-value budget has
// amount = total and no instructors line.
...
// Resolve the instructors budget for (house, month), or null when none is
// set. Mirrors budgetForHouse's specificity rule — a month-specific row's
// instructors line wins over the 'default' row — but a row whose instructors
// line is blank falls through to the default, so setting only a total for a
// single month never hides the default instructors budget.
function instructorsBudgetForHouse(budgets, house, month) {
  const list = (budgets || []).filter(b => b && b.house === house);
  const specific = list.find(b => String(b.month) === String(month));
...
// Non-blocking sanity check for the budget editor. An instructors budget
// larger than the total is ALLOWED (the two lines are tracked independently)
// but worth surfacing, so this returns a Hebrew warning string when
// instructors > total and null otherwise. It is a warning, never an error —
// validateBudget still accepts the pair. A blank instructors line never warns.
function budgetWarning(amount, instructorsAmount) {
  if (instructorsAmount === null || instructorsAmount === undefined || instructorsAmount === '') return null;
  const total = Math.max(0, num(amount));
  const inst = Math.max(0, num(instructorsAmount));
  if (inst > total) return 'תקציב המדריכים גבוה מהתקציב הכולל';
  return null;
}
...
// Cross-house worker search for the dashboard. Case-insensitive substring
// match on the worker's display name, across EVERY house. Returns one result
// per matching (worker × assignment) — so a worker placed at two houses yields
// two results, each carrying the house + role for that placement (which is what
// the UI navigates to and highlights). Orphan assignments (no matching worker)
// and workers with no assignment (nowhere to navigate) are omitted. Pure —
// presentation-free (house is the internal id; the UI maps it to a display
// name). Sorted by name then house for a stable list.
function searchWorkers(query, workers, assignments) {
  const q = String(query == null ? '' : query).trim().toLowerCase();
  if (!q) return [];
  const nameById = {};
  (workers || []).forEach(function (w) { nameById[w.id] = String(w.name || ''); });
  const out = [];
  (assignments || []).forEach(function (a) {
    const name = nameById[a.workerId];
    if (name == null) return; // orphan assignment — no worker
  ...
  out.sort(function (x, y) {
    return (x.name + ' ' + x.house).localeCompare(y.name + ' ' + y.house, 'he');
  });
  return out;
}

// ---------- first-hadracha alerts ----------

// A new staff member in a supervision-relevant role must complete their
// first hadracha within this many days of their employment start date;
// strictly MORE than this → alert.
const FIRST_HADRACHA_GRACE_DAYS = 30;

// Roles covered by first-hadracha tracking — mirror of the supervision-
// relevant roles the getGuidesForHadrachot feed publishes (see
// hadrachotFeedRole_ in apps-script/Code.gs; the two must stay in sync).
// Single-column roles, matched on the stored `role` string byte-for-byte:
// guide, house manager (stored 'מנהל/ת'), coordinator.
const HADRACHA_ROLES = ['מדריך/ה', 'מנהל/ת', 'רכז/ת'];

// A social-worker placement is stored across TWO columns: `role` is plain
// 'מטפל/ת' and the adjacent role_detail column holds 'עו"ס' — the UI merely
// displays them joined with a dash. Hand-entered cells mix quote characters
// over time, so the detail is normalized before comparing: trimmed, and the
// Hebrew gershayim ״ (U+05F4) replaced with the ASCII double quote " (U+0022)
// — then compared to the ASCII-quoted constant (same rule as
// hadrachotFeedRole_ in apps-script/Code.gs). Plain 'מטפל/ת' with any other
// or empty detail is a therapist and is NOT covered.
const HADRACHA_SOCIAL_WORKER_ROLE = 'מטפל/ת';
const HADRACHA_SOCIAL_WORKER_DETAIL = 'עו"ס';

function isHadrachaRole(role, roleDetail) {
  const r = String(role || '').trim();
  if (r === HADRACHA_SOCIAL_WORKER_ROLE) {
    const d = String(roleDetail || '').trim().replace(/״/g, '"');
    return d === HADRACHA_SOCIAL_WORKER_DETAIL;
  }
  return HADRACHA_ROLES.indexOf(r) >= 0;
}

// Whole days from startYmd to todayYmd (both 'YYYY-MM-DD'). UTC arithmetic so
// DST can never shift the count. Junk / empty input → null (callers must
// treat "unknown" as "no alert", never as "overdue").
function daysSince(startYmd, todayYmd) {
  const re = /^(\d{4})-(\d{2})-(\d{2})$/;
  const a = re.exec(String(startYmd == null ? '' : startYmd).trim());
  const b = re.exec(String(todayYmd == null ? '' : todayYmd).trim());
  if (!a || !b) return null;
...
// Guide names are matched across two apps by display name (there is no shared
// id), so normalize whitespace before comparing.
function normalizeGuideName(name) {
  return String(name == null ? '' : name).trim().replace(/\s+/g, ' ');
}

// Hadrachot status payload (relayed verbatim by GET /api/hadrachot-status) →
// the list of guide names whose FIRST hadracha is completed, or null when the
// payload shape is not recognized. null means "no usable data": the caller
// must render NOTHING — an unrecognized payload must never be read as "nobody
// completed anything" (that would flag every guide). Two accepted shapes:
//   { guides: [{ name, firstHadrachaDone: true|false, ... }] }
//   { completed: ['name', ...] }
function parseHadrachotStatus(data) {
  if (!data || typeof data !== 'object' || Array.isArray(data)) return null;
  if (Array.isArray(data.guides)) {
    return data.guides
      .filter(function (g) { return g && g.firstHadrachaDone === true; })
      .map(function (g) { return normalizeGuideName(g.name); })
      .filter(Boolean);
  }
  if (Array.isArray(data.completed)) {
    return data.completed.map(normalizeGuideName).filter(Boolean);
  }
  return null;
}

// The first-hadracha flag logic — runs ENTIRELY in the frontend, on the local
// roster joined with the completions list from the hadrachot app. Returns
// { overdue: [names], missingStart: [names] }:
//   overdue      — active staff in a supervision-relevant role (isHadrachaRole:
//                  HADRACHA_ROLES plus the two-column עו"ס social-worker rule,
//                  assignment status active) whose start_date is MORE than
//                  FIRST_HADRACHA_GRACE_DAYS days ago and whose name is not
//                  in completedNames. Exactly 30 days is NOT overdue
//                  (strict "exceeds").
//   missingStart — active staff with an empty start_date (a separate
//                  data-hygiene warning; they can never be judged overdue).
// A worker placed at two houses is counted once. completedNames must be the
// array from parseHadrachotStatus; anything else (null / undefined) means no
// usable data and yields empty lists — nothing is ever flagged blind.
function firstHadrachaFlags(workers, assignments, completedNames, today) {
  const none = { overdue: [], missingStart: [] };
  if (!Array.isArray(completedNames)) return none;
  const t = today || todayStr();
  const done = {};
  completedNames.forEach(function (n) { done[normalizeGuideName(n)] = true; });
  const workerById = {};
  ...
  const missingStart = [];
  (assignments || []).forEach(function (a) {
    if (!isHadrachaRole(a.role, a.roleDetail)) return;
    if (workerStatus(a) !== 'active') return;
    const w = workerById[a.workerId];
    if (!w) return; // orphaned assignment
    if (seen[w.id]) return;
    seen[w.id] = true;
    const start = String(w.startDate || '').trim();
    if (!start) { missingStart.push(w.name); return; }
    const days = daysSince(start, t);
    if (days == null || days <= FIRST_HADRACHA_GRACE_DAYS) return;
    if (done[normalizeGuideName(w.name)]) return;
    overdue.push(w.name);
  });

  overdue.sort(function (x, y) { return x.localeCompare(y, 'he'); });
  missingStart.sort(function (x, y) { return x.localeCompare(y, 'he'); });
  return { overdue: overdue, missingStart: missingStart };
}
...
const API = {
  ...
  WORKER_STATUSES, LEAVE_STATUSES, UNPAID_STATUSES, FINAL_SETTLEMENT_STATUS,
  workerStatus, isOnLeave, isFinalSettlement, isUnpaid,
  gmachFinished, filterGmachFinished, gmachFinishedRows,
  ...
  budgetForHouse, hasInstructorsBudget, instructorsBudgetForHouse,
  budgetWarning, budgetVariance,
  assignmentsForWorker, workerTotalCost,
  activeAbsenceForWorker,
  networkAbsenceCoverageRows,
  FIRST_HADRACHA_GRACE_DAYS, HADRACHA_ROLES, isHadrachaRole,
  HADRACHA_SOCIAL_WORKER_ROLE, HADRACHA_SOCIAL_WORKER_DETAIL,
  daysSince, normalizeGuideName,
  parseHadrachotStatus, firstHadrachaFlags,
};
