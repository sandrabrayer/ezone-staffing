...
// relevant roles the getGuidesForHadrachot feed publishes
// (HADRACHOT_FEED_ROLES in apps-script/Code.gs). The stored role string
// must match byte-for-byte: guide, social worker (ONLY the exact string
// 'מטפל/ת – עו"ס' with an en dash — plain 'מטפל/ת' is a therapist and is
// NOT covered), house manager (stored 'מנהל/ת'), coordinator.
const HADRACHA_ROLES = ['מדריך/ה', 'מטפל/ת – עו"ס', 'מנהל/ת', 'רכז/ת'];

function isHadrachaRole(role) {
  return HADRACHA_ROLES.indexOf(String(role || '').trim()) >= 0;
}
...
//   overdue      — active staff in a supervision-relevant role (HADRACHA_ROLES,
...
function firstHadrachaFlags(workers, assignments, completedNames, today) {
  ...
  const missingStart = [];
  (assignments || []).forEach(function (a) {
    if (!isHadrachaRole(a.role)) return;
