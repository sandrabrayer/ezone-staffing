...
// A termination reason is required from Phase 2 on; this is the explicit
// opt-out the UI offers, so "no reason" is deliberate. Hebrew because this
// column has always held Hebrew enum values.
...
// Coverage approval workflow. ASCII enum — the Hebrew labels live in the
// frontend only. Mirror of COVERAGE_APPROVAL_VALUES in apps-script/Code.gs.
const COVERAGE_APPROVAL_VALUES = ['pending', 'approved', 'rejected'];
// The most shifts one coverage can be worth: two per day in a 31-day month.
...
// ---------- guide shift minimums (mirror of apps-script/Code.gs) ----------
// Same ids and ranges as ezone-coordinators' Guides columns. Blank = not set
// (null, never 0). Only a מדריך/ה placement may carry a value.
const SHIFT_LABELS = ['בוקר', 'אחר צהריים', 'לילה'];
const WEEKDAY_MIN_MAX = 6;
const WEEKEND_MIN_MAX = 10;
const SHIFT_MINIMUM_ROLE = 'מדריך/ה';

function validateMinInt(v, max, field) {
  if (v === undefined || v === null) return null;
  if (typeof v !== 'number' && typeof v !== 'string') throw badRequest('bad ' + field);
  if (String(v).trim() === '') return null;
  const n = Number(v);
  if (!Number.isFinite(n) || !Number.isInteger(n) || n < 0 || n > max) throw badRequest('bad ' + field);
  return n;
}

function validateAllowedShifts(v) {
  if (v === undefined || v === null) return null;
  const parts = Array.isArray(v) ? v : (typeof v === 'string' ? v.split(',') : null);
  if (!parts) throw badRequest('bad allowedShifts');
  const seen = new Set();
  for (const p of parts) {
    if (typeof p !== 'string') throw badRequest('bad allowedShifts');
    const t = p.trim();
    if (!t) continue;
    if (!SHIFT_LABELS.includes(t)) throw badRequest('bad allowedShifts');
    seen.add(t);
  }
  const canon = SHIFT_LABELS.filter(l => seen.has(l)).join(',');
  return canon || null;
}

// Returns ONLY the keys the caller sent (key presence = "change this"), so
// Code.gs keeps a stored value the client did not mention. A non-guide
// placement gets all three explicitly cleared; a value sent for one is 400.
function validateShiftMinimums(a, role) {
  const has = k => Object.prototype.hasOwnProperty.call(a, k);
  const out = {};
  if (has('weekdayMin')) out.weekdayMin = validateMinInt(a.weekdayMin, WEEKDAY_MIN_MAX, 'weekdayMin');
  if (has('weekendMin')) out.weekendMin = validateMinInt(a.weekendMin, WEEKEND_MIN_MAX, 'weekendMin');
  if (has('allowedShifts')) out.allowedShifts = validateAllowedShifts(a.allowedShifts);
  if (role !== SHIFT_MINIMUM_ROLE) {
    if (Object.values(out).some(v => v !== null)) {
      throw badRequest('shift minimums apply to ' + SHIFT_MINIMUM_ROLE + ' only');
    }
    return { weekdayMin: null, weekendMin: null, allowedShifts: null };
  }
  return out;
}
...
function validateCoverage(c) {
  ...
  // Appended fields (Phase 2), all optional. Each is validated here AND in
  // Code.gs — defence in depth, so a caller hitting /exec directly cannot
  // write free text into an enum column.
  const replacedAssignmentId = String(c.replacedAssignmentId || '').trim();
  const role = String(c.role || '').trim();
  if (role && ROLE_OPTIONS.indexOf(role) < 0) throw badRequest('bad role');
  const shiftCount = clampInt(c.shiftCount, COVERAGE_SHIFT_COUNT_MAX);
  const rawApproval = c.approvalStatus === undefined || c.approvalStatus === null
    ? '' : String(c.approvalStatus).trim();
  const approvalStatus = rawApproval || 'pending';
  if (COVERAGE_APPROVAL_VALUES.indexOf(approvalStatus) < 0) {
    throw badRequest('bad approvalStatus');
  }
...
function validateAction(body) {
  ...
  switch (action) {
    case 'createWorker':
      // confirmDuplicate is Moran's explicit "yes, these really are two
      // different people" for the Code.gs duplicate guard. Coerced to a
      // strict boolean so no truthy string can turn the guard off by
      // accident.
      return {
        action,
        worker: validateWorker(body.worker),
    ...
    case 'moveAssignment': {
      // A TRANSFER: Code.gs ends the old placement and starts a new one with
      // the same workerId. effectiveFrom is the day the new placement
      // starts; omitted means today.
      const id = requireId(body.id);
      ...
      return { action, id, house, effectiveFrom, reasonDetail };
    }
    ...
    case 'terminateAssignment': {
      ...
      // TERMINATION_REASON_NOT_STATED, so "no reason" is a choice on the
      // record rather than a blank cell. Anything off-enum still throws.
      const reasonType = String(body.reasonType || '').trim() || TERMINATION_REASON_NOT_STATED;
    ...
    case 'deleteCoverage':
      // Code.gs CANCELS rather than deletes, keeping the row and the
      // history. The reason rides along for the audit log.
      return {
        action,
        id: requireId(body.id),
        reason: String((body && body.reason) || '').trim().slice(0, 500),
      };
...
module.exports = {
  ...
  TERMINATION_REASONS,
  TERMINATION_REASON_NOT_STATED,
  COVERAGE_APPROVAL_VALUES,
  COVERAGE_SHIFT_COUNT_MAX,
  EMPLOYMENT_TYPES,
