...
const ROLE_OPTIONS = [
  ...
  'טבח/ית',
  'איש/אשת אחזקה',
  // Marketer. Paid by commission per case (employment type
  // per_case_commission) — NEVER a therapist or a guide, so it is absent from
  // every allowlist the cross-app feeds read (see tests/marketer-role.test.js).
  'משווק/ת',
  'אחר',
];
...
// per_case_commission («עמלה לפי מקרה») has NO fixed rate and no monthly
// count: the commission is paid per closed case, outside the monthly cost
// model. Its allowed cost-field set is empty, so every rate field is
// rejected and the assignment validates with no rate at all.
...
function validateAssignment(a) {
  ...
  switch (employmentType) {
    ...
    case 'per_case_commission':
      // No rate, no count: every cost field stays 0 (foreign positives were
