...
(function (root, factory) {
  ...
  function lineForAssignment(ctx, a, opts) {
    ...
    // always been employed and the line is flagged instead.
    const rawStart = worker ? String(worker.startDate || '').trim() : '';
    const employmentStart = isDate(rawStart) ? rawStart : '';
    if (!employmentStart) missingData.push('startDate');
  ...
  function emptyTotals() {
    return { actualConfirmed: 0, estimated: 0, projectedTotal: 0, missingData: 0 };
  }

  function addToTotals(t, line) {
    if (line.source === 'estimate') t.estimated += line.cost;
    else if (line.source === 'actual') t.actualConfirmed += line.cost;
