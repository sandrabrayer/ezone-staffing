...
(function (root, factory) {
  ...
  function baseCost(a, actual) {
    ...
    // עמלה לפי מקרה: the commission is paid per case and is not a monthly
    // cost this engine can price. The base is a CONFIRMED zero — not missing
    // data — so the line never raises a «חסר תעריף» flag; an allowance, if
    // any, still counts.
    if (type === 'per_case_commission') {
      return {
        cost: allowance,
        source: 'actual',
        rule: 'COMMISSION_PER_CASE',
        basis: 'commission per case — no fixed monthly amount' + (allowance ? ' + allowance ' + allowance : ''),
        missingData: missingData,
      };
    }
