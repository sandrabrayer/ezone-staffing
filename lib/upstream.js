...
//
//   1. every upstream READ — user reads, boot warm-up, background
//      refreshes, keep-warm — waits for a slot in ONE queue capped at
//      UPSTREAM_CONCURRENCY (default 2). Two read lanes: a user request
//      always takes the next free slot ahead of queued background work.
//      WRITES have their own lane with their own slot(s)
//      (UPSTREAM_WRITE_CONCURRENCY, default 1): a save never waits behind
//      a read — not an in-flight one, not a queued one — and no amount of
//      read traffic can starve it. Writes queue only behind other writes
//      (Code.gs serializes them under LockService anyway). At most
//      concurrency + writeConcurrency Apps Script executions run at once;
...
function createUpstreamQueue(opts) {
  const o = opts || {};
  const clamp = (v, def) => Math.max(1, Math.min(6, Math.floor(Number(v) || def)));
  const limit = clamp(o.concurrency, 2);
  const writeLimit = clamp(o.writeConcurrency, 1);
  const lanes = { write: [], user: [], background: [] };
  let active = 0;       // reads in flight
  let activeWrites = 0; // writes in flight

  function start(next, isWrite) {
    if (isWrite) activeWrites++; else active++;
    Promise.resolve()
      .then(next.fn)
  ...
  function pump() {
    while (activeWrites < writeLimit && lanes.write.length) start(lanes.write.shift(), true);
    while (active < limit) {
  ...
  // lane: 'write' | 'user' (default) | 'background'.
  function run(fn, lane) {
    return new Promise((resolve, reject) => {
      const q = lane === 'write' ? lanes.write : lane === 'background' ? lanes.background : lanes.user;
  ...
  function depth() {
    return {
      active, queued: lanes.user.length + lanes.background.length, limit,
      activeWrites, queuedWrites: lanes.write.length, writeLimit,
    };
  }

  return { run, depth, limit, writeLimit };
}
